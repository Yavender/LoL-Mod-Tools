...
from lolpytools import inibin_fix
from lolpytools import troybin_fix
from lolpytools import plua
import json
...
def lolinibin2ini(infile, outfile):
    ibin = inibin.read(infile)
    inibin_fix.fix(ibin)
    writeini(ibin, outfile)


def loltroybin2troy(infile, outfile):
    ibin = inibin.read(infile)
    troybin_fix.fix(ibin)
    writeini(ibin, outfile)

def lolluaobj2lua(infile, outfile):
